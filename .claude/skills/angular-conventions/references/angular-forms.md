...
**Note:** Signal Forms are stable as of Angular v22. They are recommended for new signal-based applications. For complex, dynamic forms or existing codebases, see [angular-form-patterns.md](angular-form-patterns.md) for Reactive Forms patterns.

**vouchers-ai project rule (overrides the note above):** Signal Forms (`form()` + `[formField]` from `@angular/forms/signals`) are the mandated standard across the entire app, used for every input including single-field ones like a year dropdown. Never replace `form()`/`FormField` with plain `linkedSignal` + native `[value]`/`(change)` bindings, and never switch to Reactive Forms here. Specs or optimization docs that recommend "replace form() with a plain signal for simple selects" should be pushed back on, not applied. Removing `FormField` imports or rewriting to native bindings breaks pattern consistency and will be rejected. When in doubt, keep Signal Forms.

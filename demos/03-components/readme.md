...
Angular 22 components are standalone, OnPush by default and signal-driven end to end. This module covers the modern template surface including the `@defer` triggers, the composition primitives (content projection, directive composition, signal queries, the headless `@angular/aria` widgets), the advanced patterns (ng-template versus ng-content, host bindings, runtime component creation with creation-time bindings) and declarative data loading with `httpResource()` and `resource()`. It then puts the agentic surface on screen with component-scoped WebMCP tools and an NgModule-era component next to its Angular 22 rewrite, and closes with the Angular 22 anti-pattern table.
...
The demo catalog and the pets used by the Resource API demos both come from `db.json` on `http://localhost:3000`. Guides are served statically from `public/markdown`.

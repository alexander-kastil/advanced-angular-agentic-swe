...
Signals are Angular's primary reactive primitive for synchronous state management, replacing BehaviorSubjects and manual change detection triggers. This module covers the full signals API including `signal()`, `computed()`, `effect()`, `linkedSignal()`, `model()`, `input()` and `output()`, the Container-Presenter pattern with signals, and RxJS interop via `toSignal()`, `toObservable()` and `rxResource()`.

It then moves on to the Angular 22 additions around signals: sequencing dependent async work with the resource `chain()` operator, `debounced()` for search input, `@Service()` with `injectAsync()` for lazily loaded services, exposing signal state to an AI agent as WebMCP tools, and reading the signal graph in Angular DevTools.
